...
- `xtask` — multi-bin crate for dev tools written in Rust.
  Today's bins (14): `gen-uuid` (canonical source for every
  ...
  that), `tar-archive` (create tar archives), and
  `tokei-stats` (per-language file-size distribution via
  the tokei library crate). See
