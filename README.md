...
- `xtask` — multi-bin crate for dev tools written in Rust.
  Today's bins (13): `gen-uuid` (canonical source for every
  ...
  HuggingFace for build-time use), `system-resources`
  (print thread count + memory stats), `tar-archive`
  (create tar archives), and `tokei-stats` (per-language
  file-size distribution via the tokei library crate). See
