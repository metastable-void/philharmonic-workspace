# Archive: README.md + docs/ROADMAP.md (pre-trim, 2026-05-10)

This file preserves the full contents of `README.md` and
`docs/ROADMAP.md` as they stood before the 2026-05-10 trim. The
trim was driven by token-cost concerns: those two files are pulled
in by every push-time LLM summarizer that reads the repo, and most
of their content duplicated `CONTRIBUTING.md`, `docs/design/`, the
`.claude/skills/` files, and `CLAUDE.md`. The archive captures the
pre-trim state verbatim so context (historical phrasing, deferred
considerations, the inventory listings) is recoverable later.

See commit log entry on 2026-05-10 for the trim rationale.

---

## Pre-trim `README.md` (1354 lines)
...
**`docs-jp/`** is a docs-only submodule (not a Cargo workspace
member) containing Japanese-language documents for in-corporate
marketing and briefing to non-technical decision makers. It is
exempt from the workspace's English-only rule
([`CONTRIBUTING.md §14.6`](CONTRIBUTING.md#146-english-as-the-default))
and separated into its own submodule precisely because of that
exemption. Content is generated by Claude Code (not Codex).
...
**Build tooling** (published, submodule):
- `inline-blob` — proc-macro for embedding large binaries in `.lrodata.*` ELF sections; used by the embed impl for 2.27 GB ONNX model bundling
...
- `xtask` — multi-bin crate for dev tools written in Rust.
  Today's bins (15): `gen-uuid` (canonical source for every
  wire-format UUID we mint), `crates-io-versions` (sparse-
  index query for published crate versions), `web-fetch`
  (in-process HTTP GET, no `curl`/`wget` dependency),
  `web-post` (HTTP POST JSON payload via ureq + rustls),
  ...
  `encode-json-str` (stdin to JSON string literal),
  `hf-fetch-embed-model` (fetch ONNX embedding model from
  HuggingFace for build-time use), `resource-pressure`
  (canonical day-to-day one-line resource summary — CPU%,
  load1/cpus ratio, available/total memory, used/total swap;
  agent-facing pre-flight before resource-heavy operations
  like `pre-landing.sh` or a Codex dispatch),
...
raw `git`.** The repository has ~26 submodules; the scripts encode
...
- `./scripts/codex-status.sh` — list Codex processes spawned by
  Claude Code (via the `codex-companion.mjs` plugin shim) and
  their descendants. **Scope caveat:** standalone `codex` runs
  the user launched independently (e.g. directly in another
  terminal, the VSCode extension's app-server, or any
  non-Claude-Code-spawned session) are intentionally ignored.
  A "No Codex process running" report only confirms no
  Claude-Code-spawned Codex is active — an independent session
  may still be live and editing the workspace. `codex-logs.sh`
  has the same scope (filters on `originator: Claude Code`).
  When the user has separately dispatched Codex, do not rely on
  these scripts to confirm completion; ask the user.
...
- `./scripts/rust-lint.sh [--xtask | <crate>]` — workspace-wide
  ...
  with `RUSTDOCFLAGS=-D missing_docs`. Canonical pre-landing
  lint pass. **Workspace mode excludes xtask** via
  `--workspace --exclude xtask` (and equivalent for fmt-check
  and rustdoc): xtask has its own `target-xtask/` build cache
  and is checked separately so workspace builds and Codex runs
  share `target-main/` cleanly. Pass `--xtask` (or the crate
  name `xtask` positionally) to scope to xtask only with
  `CARGO_TARGET_DIR=target-xtask`.
- `./scripts/rust-test.sh [--xtask | <crate>] [--include-ignored|--ignored]` —
  `cargo test`, workspace (excluding xtask) or per-crate (or
  xtask via `--xtask`/`xtask` positional, with target-xtask),
  with optional `#[ignore]` control. Default skips
  `#[ignore]`'d tests; `--ignored` runs only them (use per
  modified crate to exercise its integration tests);
...
- `./scripts/pre-landing.sh [--no-ignored] [<crate>...] | --xtask` —
  ...
  modified non-xtask crate. Use this instead of running the
  three individually — one command captures the mandated flow.
  **Workspace flow excludes xtask** (xtask has its own
  `target-xtask/` and is checked separately); positional
  `xtask` is rejected with a hint to use `--xtask`. Pass
  `--xtask` to run xtask-only with `target-xtask`
  (mutually exclusive with crate args and `--no-ignored` —
  xtask has no `#[ignore]`-gated tests). Slow-by-design — run
  once per commit, not repeatedly within a single turn.
...
- `./scripts/new-submodule.sh --name <N> --remote-url <URL>
  ...
  crate with workspace-standard files, configure git
  (hooks + signing), and insert into root `Cargo.toml`.
...
to be kept in sync with every commit across 26 submodules.
...
## Pre-trim `docs/ROADMAP.md` (889 lines)
