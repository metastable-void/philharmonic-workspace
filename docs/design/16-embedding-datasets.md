...
| `source_items` | JSON array | `[{id, text, payload?}]` — the raw input |
| `corpus` | JSON array | `CorpusItem[]` — the embedded output. Absent until first embed completes. |
...
| `item_count` | i64 | Number of source items |
...
| `embed_endpoint` | Pinned | `TenantEndpointConfig` for the `embed` connector to use |

**Revision model:**

- Rev 0: created with `source_items`, `status=Created`.
  Embed job starts immediately after creation.
- Rev 1: `status=Embedding` (set when the embed job begins).
- Rev 2: `status=Ready` + `corpus` content (on success), or
  `status=Failed` (on failure). On failure the `corpus` slot
  is absent or carries the previous corpus.
- On item update: new revision with updated `source_items`,
  `status=Embedding`. The previous revision's `corpus`
  content hash is carried forward so workflows continue to
  receive the old corpus during re-embedding.

### Template data bindings

`WorkflowTemplate` gains a new optional content slot:

| Slot | Type | Description |
|------|------|-------------|
| `data_config` | JSON object | `{ "embed_datasets": { "<name>": "<dataset-uuid>" } }` |

This parallels `config` (the abstract endpoint config). The
template author assigns script-local names to dataset UUIDs.
The `data_config` slot is optional — templates without data
bindings omit it entirely, and the `data` field in the script
argument is `{}`.

Future data kinds beyond `embed_datasets` (e.g. key-value
stores, static file references) add new top-level keys to
`data_config` without changing the schema shape.
...
2. For each entry in `data_config.embed_datasets`:
   a. Look up the `EmbeddingDataset` by public UUID.
   b. If the dataset is Ready, read the `corpus` content blob.
   c. If the dataset is Embedding, read the previous revision's
      `corpus` (if any). If no previous corpus exists, omit the
      entry.
   d. If the dataset is Created, Failed, or Retired, omit the
      entry.
   e. Deserialize the corpus JSON and add it to
      `data.embed_datasets.<name>`.
...
2. API stores the new revision (`status=Embedding`, updated
   `source_items`, carried-forward `corpus` from previous
   revision if any).
3. API lowers the `embed_endpoint` config (produces the
   `MechanicsConfig` with the embed connector's `HttpEndpoint`).
4. API sends a `MechanicsJob` to the mechanics worker:
   - `module_source`: a built-in embed script (compiled into the
     API binary, not user-authored).
   - `arg`: `{ items: [{id, text, payload?}] }`
   - `config`: the lowered `MechanicsConfig`.
5. The built-in script iterates over items, batches texts, calls
   the embed connector, assembles `CorpusItem[]` from the
   responses.
6. The mechanics worker returns the corpus to the API.
7. API stores a new revision with `status=Ready` and the
   `corpus` content blob. On failure, `status=Failed`.
...
- `POST /v1/embed-datasets` — create dataset.
  ...
  Returns: `{dataset_id}`. Triggers embed job.
...
6. Ephemeral embed job (built-in script + direct
   lowerer/executor call from API).
7. WebUI pages + i18n.

Steps 1–5 can land without the embed job (datasets would be
created with `status=Ready` and admin-supplied corpus for
testing). Step 6 closes the loop. Step 7 is parallel-safe
after step 3.
